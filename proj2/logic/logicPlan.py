...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    s1 = PropSymbolExpr("PacmanAlive",time=1) % ((PropSymbolExpr("PacmanAlive",time=0) & ~PropSymbolExpr("PacmanKilled",time=0))|
                                            (~PropSymbolExpr("PacmanAlive",time=0) & PropSymbolExpr("PacmanBorn",time=0)))
    
    s2 = ~(PropSymbolExpr("PacmanAlive",time=0) & PropSymbolExpr("PacmanBorn",time=0))
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # combinations = [disjoin(conjoin(i,~j),conjoin(~i,j)) for i,j in list(itertools.combinations(literals,2))]
    combinations = [disjoin(~i,~j) for i,j in list(itertools.combinations(literals,2))]
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return conjoin(atMostOne(literals),disjoin(literals))
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    #     successors.append(conjoin(possible_cause)
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for coords in all_coords:
        x,y = coords
        if walls_grid[x][y] == 1:
            pacphysics_sentences.append(PropSymbolExpr(wall_str,x,y,time=t) >> PropSymbolExpr(pacman_str,x,y,time=t))
        if coords in non_outer_wall_coords:
            pacphysics_sentences.append(PropSymbolExpr(pacman_str,x,y,time=t))
            pacphysics_sentences.append(sensorModel(t,non_outer_wall_coords))

            flag = False
            if walls_grid[x][y+1] != 1:
                pacphysics_sentences.append(PropSymbolExpr('South', time=t))
                flag = True
            if walls_grid[x][y-1] != 1:
                pacphysics_sentences.append(PropSymbolExpr('North', time=t))
                flag = True
            if walls_grid[x+1][y] != 1:
                pacphysics_sentences.append(PropSymbolExpr('West', time=t))
                flag = True
            if walls_grid[x-1][y] != 1:
                pacphysics_sentences.append(PropSymbolExpr('East', time=t))
                flag = True
                
            if flag: pacphysics_sentences.append(successorAxioms(t,walls_grid,non_outer_wall_coords))
